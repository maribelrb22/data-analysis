...
Además, obtenemos el p-value correspondiente a cada una de ellas. Las variables con dos o tres asteriscos aportan bastante relevancia como predictores;
sin embargo, las variables que no poseen ninguno o uno son de menor relevancia.

A continuación, procedemos a predecir las clases del conjunto de entrenamiento y de validación. Tomamos como umbral 0,5, de forma que
si la probabilidad queda por encima de dicho umbral es que es comestible, pero si queda por debajo es que es venenoso.
...
confusionM <- table(training_set$class, pred_train)
confusionM
...
confusionM <- table(test_set$class, pred_test)
confusionM

accuracy_rl <- sum(diag(confusionM)) / sum(confusionM)
accuracy_rl
```
...
plot(perf_rl_roc)
```
Observando la curva ROC resultante podemos comentar que se mantiene por encima de la diagonal, lo que es buena señal, pero se aproxima a ella, pudiendo haber
proporcionado resultados mejores.
...
```{r}
library(rattle)
tree2 <- rpart(class ~ ., training_set, method = "class", 
...
Una vez tenemos el resultado para el valor de precisión en la predicción una vez aplicado el árbol de decisión, 83%,
pasamos a aplicar de forma visual la construcción de la gráfica de la curva ROC y el cálculo del AUC. En este caso,
continúa siendo mejor resultado que el obtenido mediante la regresión logística, pero quedando por debajo que el
...
### Random Forests classiffier<a name="random"></a>
...
Definimos la curva ROC para este caso y realizamos el cálculo del área bajo la curva, los cuales nos simbolizan su altísima precisión.
...
Comenzamos a aplicar el clasificador Máquina de Soporte Vectorial haciendo uso de la función svm().
...
library(e1071)
set.seed(18)
svm_classiffier <- svm(class ~ ., data = training_set, 
...
Como se puede observar el valor de la predicción en este caso resulta ser del 95%.
...
Una vez aplicados los cinco distintos métodos de clasificación sobre nuestro dataset tras realizar el preprocesamiento, podemos concluir diciendo que los clasificadores de KNN y Random Forest son los que poseen mayor precisión
y, por tanto, menor error de predicción. Sin embargo, con los resultados obtenidos por parte de ambos podemos detectar un posible sobreajuste, no beneficiando al modelo. Por esta razón, creemos que resulta más beneficioso sacrificar
parte del valor de precisión, teniendo en cuenta algunos valores de falsos positivos y falsos negativos, como ocurre en el caso del clasificador de la Máquina de Soporte Vectorial, aprovechando así su capacidad de mayor generalización.
...
Para poder trabajar con algoritmos no supervisados será necesario que las variables sean numéricas. Por lo tanto, se eliminarán las variables categóricas del dataset.
...
A continuación, se representará la distribución de los datos en función de los clusters obtenidos.
```{r}
...
Se puede observar que los champiñones de menor tamaño (incluyendo diámetro, altura y anchura) pertenecen al cluster 1 y los de mayor tamaño pertenecen al cluster 2.

Considerando el clúster 1 como champiñones venenosos y el clúster 2 como champiñones comestibles, se puede observar que los champiñones comestibles son más grandes que los comestibles. Comprobaremos a través de medidas de bondad externas la eficacia del clustering.
```{r}
real_classes <- ifelse(mushroom$class == "e", 1, 2)
predicted_classes <- km_model$cluster
predicted_classes <- as.numeric(predicted_classes)
```

Calculamos el accuracy del clustering a través de los datos externos, para representar la proporción de datos correctamente clasificados. Obtenemos un accuracy del 69,7%, lo que indica que el clustering es eficaz.
```{r}
...
A partir de este momento, hemos decidido modificar el dataset actual debido a que para aplicar diversas técnicas como "silhouette" o "dendrogram" (para el caso de clustering jerárquico) es necesario que el dataset sea de menor tamaño.
...
Ya podemos trabajar con el dataset reducido. Comprobamos que el dataset tiene 611 ejemplos y 3 atributos.
```{r}
...
```{r}
km_sm_model <- kmeans(smaller_df, center = 2, nstar = 20)
...
Cortaremos el dendrograma en un punto que nos interese para obtener los clusters. En este caso, hemos decidido cortar el dendrograma en 70.
```{r}
cut <- 0.9
...
Para obtener el número óptimo de cluster, haremos uso de la medida interna de bonda silhouette. Para ello, utilizaremos la función fviz_nbclust de factoextra.

```{r}
fviz_nbclust(smaller_df, FUNcluster = hcut, method = "silhouette")
```

En este caso, hemos obtenido que el número óptimo de clusters sería 3.
...
Podemos observar que el número de clusters obtenidos ha sido 3 debido a que existen datos muy dispersos que no se pueden agrupar en un mismo cluster.
